...
function ColumnsTable({
  ...
  const store = useMemo(
    () =>
      new CustomStore({
        ...
        load: function ({ skip = 0, take = 20, filter, userData }) {
          ...
          return (
            userData &&
            apiMethod({ ...apiPayload, skip, take, FilterQuery }).then(
              (data) => {
    ...
    [apiKey, apiMethod, apiPayload, otherMethod, sqlStatementGenerator]
  );
  ...
  return (
    <React.Fragment>
      <DataGrid
        ...
        selection={{ mode: selectionMode, allowSelectAll: allowSelectAllMode }}
        ...
        {colAttributes?.length > 0 &&
          colAttributes.map((col, index) => {
            return (
              ...
              >
                {col.incol?.length > 0 &&
                  col.incol.map((colin, indexin) => {
                    return (
                      <Column
                        key={indexin}
                        name={colin.field}
                        dataType={colin.dataType || "string"}
                        visible={colin.isVisable}
                        dataField={colin.field}
                        caption={
                          i18n.language === "en"
                            ? colin.captionEn ?? colin.caption
                            : colin.caption
                        }
                        alignment={colin.alignment || "right"}
                        format={colin.format}
                        cssClass={colin.cssClass}
                        grouped={colin.grouped}
                        groupIndex={colin.groupIndex}
                        autoExpandGroup={false}
                        onFilterValuesChange={onFilterValuesChange}
                        allowFiltering={colin.HideFilter ? false : true}
                        // width={
                        //     col.widthRatio
                        //         ? `${col.widthRatio}px`
                        //         : "150px"
